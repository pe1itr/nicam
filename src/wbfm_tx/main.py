...
from .audio_source import constant_audio_blocks, list_audio_devices, read_wav, wav_blocks
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--source", choices=("wav", "file", "device", "stream", "udp", "silence"), help="Audiobron.")
    ...
    parser.add_argument("--udp-url", help="UDP input URL voor --source udp, bijvoorbeeld udp://0.0.0.0:7355.")
    parser.add_argument("--no-stream-reconnect", action="store_true", help="Schakel ffmpeg reconnect/herstart uit.")
    parser.add_argument(
        "--no-stream-silence",
        action="store_true",
        help="Verouderde optie; TX vult ontbrekende input altijd met stilte.",
    )
    parser.add_argument("--stream-reconnect-delay", type=float, help="Wachttijd in seconden vóór ffmpeg herstart.")
    parser.add_argument("--audio-read-timeout-ms", type=int, help="Wachttijd vóór stilte-invulling bij input-underrun.")
    parser.add_argument("--audio-buffer-ms", type=int, help="PCM buffer voor netwerk-/ffmpeg-inputs.")
...
def apply_cli_overrides(config: dict, args: argparse.Namespace) -> dict:
    ...
    if args.udp_url:
        config["audio"]["udp_url"] = args.udp_url
    ...
    if args.audio_read_timeout_ms is not None:
        config["audio"]["read_timeout_ms"] = args.audio_read_timeout_ms
    if args.audio_buffer_ms is not None:
        config["audio"]["buffer_ms"] = args.audio_buffer_ms
...
def make_source(config: dict):
    ...
    if source in ("file", "device", "stream", "udp", "silence"):
        stream_url = audio_cfg.get("stream_url")
        audio_file = str(audio_cfg["input_file"]) if audio_cfg.get("input_file") else None
        return constant_audio_blocks(
            source,
            ...
            udp_url=str(audio_cfg.get("udp_url")) if audio_cfg.get("udp_url") else None,
            reconnect=bool(audio_cfg.get("stream_reconnect", True)),
            reconnect_delay_s=float(audio_cfg.get("stream_reconnect_delay_s", 2.0)),
            read_timeout_s=float(audio_cfg.get("read_timeout_ms", 120)) / 1000.0,
            buffer_ms=int(audio_cfg.get("buffer_ms", 1200)),
        )
    if source == "silence":
